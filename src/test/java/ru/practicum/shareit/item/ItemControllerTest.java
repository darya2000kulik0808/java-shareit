...
public class ItemControllerTest {
    ...
    void shouldAdd() throws Exception {
        ...
        itemInDto = itemInDtoBuilder.name("").build();
        json = mapper.writeValueAsString(itemInDto);
        mvc.perform(post(URL)
                        .header("X-Sharer-User-Id", 1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andDo(print())
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error", containsString("Произошла ошибка!")))
                .andExpect(jsonPath("$.description", containsString("Название не может быть пустым")));

        itemInDto = itemInDtoBuilder.name("name").description("").build();
        json = mapper.writeValueAsString(itemInDto);
        mvc.perform(post(URL)
                        .header("X-Sharer-User-Id", 1)
                        .contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error", containsString("Произошла ошибка!")))
                .andExpect(jsonPath("$.description", containsString("Описание не может быть пустым")));
        mvc.perform(post(URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andDo(print())
    ...
    void shouldAddComment() throws Exception {
        ...
        mvc.perform(post(URL + "/1/comment")
                ...
                .andDo(print())
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.description", containsString(error)));
    }
}
